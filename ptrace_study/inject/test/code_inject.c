...
int pid_write(pid_t pid, uint8_t *dest, uint8_t *data, size_t size) {
    uint64_t i, j, remain;
    uint8_t *laddr;
...
int main(int argc, char **argv) {
    ...
    Elf64_Ehdr *ehdr;
    if (argc < 3) {
        printf("%d Usage: %s <program> <function>\n", __LINE__, argv[0]);
        exit(1);
    }
...
//        printf("%d wrong WSTOPSIG(status)=%d\n", __LINE__, WSTOPSIG(status));
//        exit(1);
//    }
    printf("line : %d", __LINE__);
