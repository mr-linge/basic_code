...
int main(int argc, char **argv){
	handle_t h;
	unsigned long shellcode_size = f2 - f1; //本地两个函数 地址差 即是 上一个函数所占内存大小
	int i, fd, status;
	...
	//    exit(1);
	getdata(h.pid, h.base, origcode, shellcode_size);

	// if(pid_write(h.pid, (void*)h.base, (void*)h.shellcode, shellcode_size) < 0)
	//     exit(1);
	putdata(h.pid, h.base, h.shellcode, shellcode_size);
